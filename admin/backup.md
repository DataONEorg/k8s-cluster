...
```
velero schedule create k8s-dev-full-daily --schedule="0 16 * * *" --ttl 2160h0m0s --snapshot-move-data
...
velero schedule create k8s-prod-full-monthly --schedule="0 3 1 * *" --ttl 26280h0m0s --snapshot-move-data
```
...
velero install \
  ...
  --bucket k8s-dev \
  --secret-file /Users/outin/.aws/k8s-dev-nhdc \
...
velero install \
  ...
  --plugins velero/velero-plugin-for-aws:v1.14.1 \
  --bucket k8s-prod \
  --secret-file /Users/outin/.aws/k8s-prod-nhdc \
  --backup-location-config region=default,s3Url=https://s3.nceas.ucsb.edu,s3ForcePathStyle=true \
  --snapshot-location-config region=default \
  --use-node-agent \
  --use-volume-snapshots=true \
